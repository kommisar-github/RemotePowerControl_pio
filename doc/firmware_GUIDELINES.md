...
- **C5a — Snapshot `volatile` before passing by pointer to C library functions.** `volatile T*` is not implicitly convertible to `const T*` in C++ — dropping `volatile` is a HARD compile error (a qualification conversion may add cv-qualifiers, never remove `volatile`; `-fpermissive` only downgrades it to a warning). When a volatile global (e.g. `boottime`) must be passed to a function taking `const T*` (e.g. `ctime(const time_t*)`), snapshot to a non-volatile local first: `time_t bt = boottime; ctime(&bt);` — this also preserves correct volatile read semantics (single read into the local). Do NOT cast away volatile (`(time_t*)&boottime`) — it compiles but reintroduces a stale/torn-read risk. Scope: only the volatile globals (`boottime`/`timeset_cnt`) need this; sibling `ctime(&test.*_timestamp)` calls (`main.cpp:420-435`) are fine because those `Test` members are not volatile. Encountered v1.0.1: `send_info()` + `main_debug_info()` (`src/main.cpp:373, :467`) failed to compile after F-05 made `boottime` volatile. (v1.0.1 fix phase; `/review` APPROVE)
